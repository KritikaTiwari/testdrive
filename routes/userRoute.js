...
router.post("/get-all-testdrive", async (req, res) => {
  try {
    const userId = req.body.userId;
    const testdrives = await TestDrive.find({ userId })
      .lean()
      .populate("sales_executive", "_id name");

    testdrives.forEach((testdrive) => {
      if (testdrive.sales_executive) {
        testdrive.sales_executive = testdrive.sales_executive.name;
      }
    });
...
router.post("/testdrive-permission", authMiddleware, async (req, res) => {
  try {
    ...
    const existingTestDrive = await TestDrive.findOne({
      chassis_number,
      $or: [
        { km_in: { $eq: 0 }, status: "Approved" },
        { km_in: { $eq: 0 }, status: "pending" },
      ],
    });

    if (existingTestDrive) {
      if (existingTestDrive.status === "Approved") {
        return res.status(200).send({
          success: false,
          message: "Car already gone for test drive",
        });
      } else if (existingTestDrive.status === "pending") {
        return res.status(200).send({
          success: false,
          message: "This car has been already requested by someone",
        });
      }
    }
    ...
    const newtestdrive = new TestDrive({
      ...req.body,
      status: "pending",
      sales_executive: req.body.sales_executive,
    });
    ...
    res.status(500).send({
      success: false,
      message: "Error sending test drive request",
      error,
    });
  }
});
